...
use OAuth3\Controller\ResourceControllerInterface;
use OAuth3\Controller\ResourceController;
use OAuth3\OpenID\Controller\UserInfoControllerInterface;
use OAuth3\OpenID\Controller\UserInfoController;
use OAuth3\OpenID\Controller\AuthorizeController as OpenIDAuthorizeController;
use OAuth3\OpenID\ResponseType\AuthorizationCode as OpenIDAuthorizationCodeResponseType;
use OAuth3\OpenID\Storage\AuthorizationCodeInterface as OpenIDAuthorizationCodeInterface;
use OAuth3\OpenID\GrantType\AuthorizationCode as OpenIDAuthorizationCodeGrantType;
use OAuth3\Controller\AuthorizeControllerInterface;
use OAuth3\Controller\AuthorizeController;
use OAuth3\Controller\TokenControllerInterface;
use OAuth3\Controller\TokenController;
use OAuth3\ClientAssertionType\ClientAssertionTypeInterface;
use OAuth3\ClientAssertionType\HttpBasic;
use OAuth3\ResponseType\ResponseTypeInterface;
use OAuth3\ResponseType\AuthorizationCode as AuthorizationCodeResponseType;
use OAuth3\ResponseType\AccessToken;
use OAuth3\ResponseType\JwtAccessToken;
use OAuth3\OpenID\ResponseType\CodeIdToken;
use OAuth3\OpenID\ResponseType\IdToken;
use OAuth3\OpenID\ResponseType\IdTokenToken;
use OAuth3\TokenType\TokenTypeInterface;
use OAuth3\TokenType\Bearer;
use OAuth3\GrantType\GrantTypeInterface;
...
use OAuth3\Storage\ClientInterface;
use OAuth3\Storage\JwtAccessToken as JwtAccessTokenStorage;
use OAuth3\Storage\JwtAccessTokenInterface;
use InvalidArgumentException;
use LogicException;

/**
* Server class for OAuth3
* This class serves as a convience class which wraps the other Controller classes
*
* @see \OAuth3\Controller\ResourceController
* @see \OAuth3\Controller\AuthorizeController
* @see \OAuth3\Controller\TokenController
*/
    ...
    UserInfoControllerInterface
{
    ...
    protected $storageMap = array(
        'access_token' => 'OAuth3\Storage\AccessTokenInterface',
        'authorization_code' => 'OAuth3\Storage\AuthorizationCodeInterface',
        'client_credentials' => 'OAuth3\Storage\ClientCredentialsInterface',
        'client' => 'OAuth3\Storage\ClientInterface',
        'refresh_token' => 'OAuth3\Storage\RefreshTokenInterface',
        'user_credentials' => 'OAuth3\Storage\UserCredentialsInterface',
        'user_claims' => 'OAuth3\OpenID\Storage\UserClaimsInterface',
        'public_key' => 'OAuth3\Storage\PublicKeyInterface',
        'jwt_bearer' => 'OAuth3\Storage\JWTBearerInterface',
        'scope' => 'OAuth3\Storage\ScopeInterface',
    );
    ...
    protected $responseTypeMap = array(
        'token' => 'OAuth3\ResponseType\AccessTokenInterface',
        'code' => 'OAuth3\ResponseType\AuthorizationCodeInterface',
        'id_token' => 'OAuth3\OpenID\ResponseType\IdTokenInterface',
        'id_token token' => 'OAuth3\OpenID\ResponseType\IdTokenTokenInterface',
        'code id_token' => 'OAuth3\OpenID\ResponseType\CodeIdTokenInterface',
    );

    /**
     * @param mixed                        $storage             (array or OAuth3\Storage) - single object or array of objects implementing the
     ...
     * @param array                        $grantTypes          An array of OAuth3\GrantType\GrantTypeInterface to use for granting access tokens
    ...
    protected function createDefaultAuthorizeController()
    {
        if (!isset($this->storages['client'])) {
            throw new \LogicException('You must supply a storage object implementing \OAuth3\Storage\ClientInterface to use the authorize server');
        }
    ...
    protected function createDefaultTokenController()
    {
            ...
            foreach ($this->grantTypes as $grantType) {
                if (!$grantType instanceof ClientAssertionTypeInterface) {
                    if (!isset($this->storages['client_credentials'])) {
                        throw new \LogicException('You must supply a storage object implementing OAuth3\Storage\ClientCredentialsInterface to use the token server');
                    }
        ...
        if (!isset($this->storages['client'])) {
            throw new LogicException("You must supply a storage object implementing OAuth3\Storage\ClientInterface to use the token server");
        }
    ...
    protected function createDefaultResourceController()
    {
        if ($this->config['use_jwt_access_tokens']) {
            ...
            throw new \LogicException('You must supply a storage object implementing OAuth3\Storage\AccessTokenInterface or use JwtAccessTokens to use the resource server');
        }
    ...
    protected function createDefaultUserInfoController()
    {
        if ($this->config['use_jwt_access_tokens']) {
            ...
            throw new \LogicException('You must supply a storage object implementing OAuth3\Storage\AccessTokenInterface or use JwtAccessTokens to use the UserInfo server');
        }

        if (!isset($this->storages['user_claims'])) {
            throw new \LogicException('You must supply a storage object implementing OAuth3\OpenID\Storage\UserClaimsInterface to use the UserInfo server');
        }
    ...
    protected function getDefaultResponseTypes()
    {
        ...
        if (isset($this->storages['authorization_code'])) {
            ...
            if ($this->config['use_openid_connect']) {
                if (!$this->storages['authorization_code'] instanceof OpenIDAuthorizationCodeInterface) {
                    throw new \LogicException('Your authorization_code storage must implement OAuth3\OpenID\Storage\AuthorizationCodeInterface to work when "use_openid_connect" is true');
                }
        ...
        if (count($responseTypes) == 0) {
            throw new \LogicException('You must supply an array of response_types in the constructor or implement a OAuth3\Storage\AuthorizationCodeInterface storage object or set "allow_implicit" to true and implement a OAuth3\Storage\AccessTokenInterface storage object');
        }
    ...
    protected function createDefaultJwtAccessTokenStorage()
    {
        if (!isset($this->storages['public_key'])) {
            throw new \LogicException('You must supply a storage object implementing OAuth3\Storage\PublicKeyInterface to use crypto tokens');
        }
        $tokenStorage = null;
    ...
    {
        if (!isset($this->storages['access_token'])) {
            throw new LogicException("You must supply a response type implementing OAuth3\ResponseType\AccessTokenInterface, or a storage object implementing OAuth3\Storage\AccessTokenInterface to use the token server");
        }
    ...
    {
        if (!isset($this->storages['user_claims'])) {
            throw new LogicException("You must supply a storage object implementing OAuth3\OpenID\Storage\UserClaimsInterface to use openid connect");
        }
        if (!isset($this->storages['public_key'])) {
            throw new LogicException("You must supply a storage object implementing OAuth3\Storage\PublicKeyInterface to use openid connect");
        }
