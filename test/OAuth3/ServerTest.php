...
use OAuth3\ResponseType\AuthorizationCode;
...
{
    /**
     * @expectedException LogicException OAuth3\Storage\ClientInterface
     **/
    ...
    /**
     * @expectedException LogicException OAuth3\Storage\AccessTokenInterface
     **/
    public function testGetAuthorizeControllerWithNoAccessTokenStorageThrowsException()
    {
        ...
        $server = new Server();
        $server->addStorage($this->getMock('OAuth3\Storage\ClientInterface'));
        $server->getAuthorizeController();
    }
    ...
    public function testGetAuthorizeControllerWithClientStorageAndAccessTokenResponseType()
    {
        ...
        $server = new Server();
        $server->addStorage($this->getMock('OAuth3\Storage\ClientInterface'));
        $server->addResponseType($this->getMock('OAuth3\ResponseType\AccessTokenInterface'));
    ...
    public function testGetAuthorizeControllerWithClientStorageAndAuthorizationCodeResponseType()
    {
        ...
        $server = new Server();
        $server->addStorage($this->getMock('OAuth3\Storage\ClientInterface'));
        $server->addResponseType($this->getMock('OAuth3\ResponseType\AuthorizationCodeInterface'));
    ...
    public function testGetAuthorizeControllerWithClientStorageAndAccessTokenStorageThrowsException()
    {
        ...
        $server = new Server();
        $server->addStorage($this->getMock('OAuth3\Storage\ClientInterface'));
        $server->addStorage($this->getMock('OAuth3\Storage\AccessTokenInterface'));
    ...
    public function testGetAuthorizeControllerWithClientStorageAndAuthorizationCodeStorage()
    {
        ...
        $server = new Server();
        $server->addStorage($this->getMock('OAuth3\Storage\ClientInterface'));
        $server->addStorage($this->getMock('OAuth3\Storage\AuthorizationCodeInterface'));
    ...
    /**
     * @expectedException LogicException OAuth3\Storage\ClientCredentialsInterface
     **/
    public function testGetTokenControllerWithNoClientCredentialsStorageThrowsException()
    {
        $server = new Server();
        $server->addStorage($this->getMock('OAuth3\Storage\UserCredentialsInterface'));
        $server->getTokenController();
    }
    ...
    {
        $server = new Server();
        $server->addStorage($this->getMock('OAuth3\Storage\ClientCredentialsInterface'));
        $server->getTokenController();
    }
    ...
    public function testGetTokenControllerAccessTokenStorageAndClientCredentialsStorageAndGrantTypes()
    {
        ...
        $server->addStorage($this->getMock('OAuth3\Storage\AccessTokenInterface'));
        $server->addStorage($this->getMock('OAuth3\Storage\ClientCredentialsInterface'));
        $server->addGrantType($this->getMockBuilder('OAuth3\GrantType\AuthorizationCode')->disableOriginalConstructor()->getMock());
        $server->getTokenController();
    }
    ...
    /**
     * @expectedException InvalidArgumentException OAuth3\Storage\AccessTokenInterface
     **/
    ...
    {
        $server = new Server();
        $server->addStorage($this->getMock('OAuth3\Storage\AccessTokenInterface'), 'nonexistant_storage');
    }

    /**
     * @expectedException InvalidArgumentException OAuth3\Storage\AuthorizationCodeInterface
     **/
    public function testAddingStorageWithInvalidKeyStorageCombination()
    {
        $server = new Server();
        $server->addStorage($this->getMock('OAuth3\Storage\AccessTokenInterface'), 'authorization_code');
    }

    public function testAddingStorageWithValidKeyOnlySetsThatKey()
    {
        $server = new Server();
        $server->addStorage($this->getMock('OAuth3\Storage\Memory'), 'access_token');
    ...
    {
        $server = new Server();
        $memory = $this->getMock('OAuth3\Storage\Memory');
    ...
    {
        $server = new Server();
        $pdo = $this->getMockBuilder('OAuth3\Storage\Pdo')
    ...
    {
        $storage = $this->getMock('OAuth3\Storage\Memory');
        $storage
          ->expects($this->any())
    ...
    public function testCustomClientAssertionType()
    {
        ...
        $clientAssertionType = $this->getMock('OAuth3\ClientAssertionType\ClientAssertionTypeInterface', array('validateRequest', 'getClientId'));
        $clientAssertionType
            ->expects($this->once())
     ...
     *
     * @see test/OAuth3/GrantType/RefreshTokenTest::testValidRefreshTokenWithNewRefreshTokenInResponse
     **/
    ...
    /**
     * @expectedException InvalidArgumentException OAuth3\ResponseType\AuthorizationCodeInterface
     **/
    public function testAddingUnknownResponseTypeThrowsException()
    {
        $server = new Server();
        $server->addResponseType($this->getMock('OAuth3\ResponseType\ResponseTypeInterface'));
    }

    /**
     * @expectedException LogicException OAuth3\Storage\PublicKeyInterface
     **/
    public function testUsingJwtAccessTokensWithoutPublicKeyStorageThrowsException()
    {
        ...
        $server->addGrantType($this->getMock('OAuth3\GrantType\GrantTypeInterface'));
        $server->addStorage($this->getMock('OAuth3\Storage\ClientCredentialsInterface'));
        $server->addStorage($this->getMock('OAuth3\Storage\ClientCredentialsInterface'));
    ...
    {
        $pubkey = $this->getMock('OAuth3\Storage\PublicKeyInterface');
        $server = new Server(array($pubkey), array('use_jwt_access_tokens' => true));

        $this->assertNotNull($server->getResourceController());
        $this->assertInstanceOf('OAuth3\Storage\PublicKeyInterface', $server->getStorage('public_key'));
    }
    ...
    {
        $pubkey = $this->getMock('OAuth3\Storage\PublicKeyInterface');
        $client = $this->getMock('OAuth3\Storage\ClientInterface');
        $server = new Server(array($pubkey, $client), array('use_jwt_access_tokens' => true, 'allow_implicit' => true));
        $this->assertNotNull($server->getAuthorizeController());

        $this->assertInstanceOf('OAuth3\ResponseType\JwtAccessToken', $server->getResponseType('token'));
    }
    ...
    {
        $client = $this->getMock('OAuth3\Storage\ClientInterface');
        $userclaims = $this->getMock('OAuth3\OPenID\Storage\UserClaimsInterface');
    ...
    public function testUsingOpenIDConnectWithIssuerPublicKeyAndUserClaimsIsOkay()
    {
        ...
        $this->assertInstanceOf('OAuth3\OpenID\ResponseType\IdTokenInterface', $server->getResponseType('id_token'));
        $this->assertNull($server->getResponseType('id_token token'));
    }

    /**
     * @expectedException LogicException OAuth3\ResponseType\AccessTokenInterface
     **/
    ...
    public function testUsingOpenIDConnectWithAllowImplicitAndUseJwtAccessTokensIsOkay()
    {
        ...
        $this->assertInstanceOf('OAuth3\OpenID\ResponseType\IdTokenInterface', $server->getResponseType('id_token'));
        $this->assertInstanceOf('OAuth3\OpenID\ResponseType\IdTokenTokenInterface', $server->getResponseType('id_token token'));
    }
    ...
    public function testUsingOpenIDConnectWithAllowImplicitAndAccessTokenStorageIsOkay()
    {
        ...
        $userclaims = $this->getMock('OAuth3\OpenID\Storage\UserClaimsInterface');
        $pubkey = $this->getMock('OAuth3\Storage\PublicKeyInterface');
        $token = $this->getMock('OAuth3\Storage\AccessTokenInterface');
    ...
    public function testUsingOpenIDConnectWithAllowImplicitAndAccessTokenResponseTypeIsOkay()
    {
        ...
        $token = $this->getMock('OAuth3\ResponseType\AccessTokenInterface');
    ...
    /**
     * @expectedException LogicException OAuth3\OpenID\Storage\AuthorizationCodeInterface
     **/
    public function testUsingOpenIDConnectWithAuthorizationCodeStorageThrowsException()
    {
        $client = $this->getMock('OAuth3\Storage\ClientCredentialsInterface');
        $userclaims = $this->getMock('OAuth3\OpenID\Storage\UserClaimsInterface');
        $pubkey = $this->getMock('OAuth3\Storage\PublicKeyInterface');
        $token = $this->getMock('OAuth3\Storage\AccessTokenInterface');
        $authcode = $this->getMock('OAuth3\Storage\AuthorizationCodeInterface');
        ...
        $this->assertInstanceOf('OAuth3\OpenID\GrantType\AuthorizationCode', $server->getGrantType('authorization_code'));
    }
    ...
    public function testUsingOpenIDConnectWithOpenIDAuthorizationCodeStorageCreatesOpenIDAuthorizationCodeGrantType()
    {
        ...
        $pubkey = $this->getMock('OAuth3\Storage\PublicKeyInterface');
        $token = $this->getMock('OAuth3\Storage\AccessTokenInterface');
        $authcode = $this->getMock('OAuth3\OpenID\Storage\AuthorizationCodeInterface');
    ...
    {
        $responseType = $this->getMock('OAuth3\OpenID\ResponseType\IdTokenTokenInterface');
    ...
    {
        $server = new Server();
        $server->addGrantType(new \OAuth3\GrantType\AuthorizationCode($this->getMock('OAuth3\Storage\AuthorizationCodeInterface')));
    ...
    {
        $server = new Server();
        $server->addGrantType(new \OAuth3\GrantType\AuthorizationCode($this->getMock('OAuth3\Storage\AuthorizationCodeInterface')), 'ac');
    ...
    {
        $server = new Server();
        $server->addGrantType(new \OAuth3\GrantType\AuthorizationCode($this->getMock('OAuth3\Storage\AuthorizationCodeInterface')), 42);
