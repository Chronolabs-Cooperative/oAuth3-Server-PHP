...
class Bootstrap
{
    ...
    {
        try {
            $pdo = new \PDO('pgsql:host=localhost;dbname=OAuth3_server_php', 'postgres');
        ...
        if (!$this->mongo) {
            if (class_exists('MongoClient')) {
                ...
                if ($this->testMongoConnection($mongo)) {
                    $db = $mongo->OAuth3_server_php_legacy;
                    $this->removeMongo($db);
                    $this->createMongo($db);
    ...
    public function getCassandraStorage()
    {
        if (!$this->cassandra) {
            if (class_exists('phpcassa\ColumnFamily')) {
                $cassandra = new \phpcassa\Connection\ConnectionPool('OAuth3_test', array('127.0.0.1:9160'));
    ...
    {
        $pdo->exec('CREATE DATABASE OAuth3_server_php');
        $pdo->exec('USE OAuth3_server_php');
        $this->runPdoSql($pdo);
    }

    private function removeMysqlDb(\PDO $pdo)
    {
        $pdo->exec('DROP DATABASE IF EXISTS OAuth3_server_php');
    }
    ...
    private function createPostgresDb()
    {
        ...
        `createdb -O postgres OAuth3_server_php`;
    }
    ...
    {
        if (trim(`psql -l | grep OAuth3_server_php | wc -l`)) {
            `dropdb OAuth3_server_php`;
        }
    }
